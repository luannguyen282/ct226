<?php 
	...
	if (isset($_SESSION['rp'])) {
		echo "<script>alert('".$_SESSION['rp']."');</script>";
		unset($_SESSION['rp']);
	}
?>
...
<body>
	...
	<div id="main">
		...
		<div id="filter">
			<form >
				<div style="float: left;"><b>|| Bộ lọc:</b></div>
				<label for="khoa">Khoa: </label>
				<select id="khoa" name="khoa">
					<option class="option" value="all">Tất cả</option>
					<?php 
						$sql = "SELECT * FROM `khoa`;";
						$khoa = mysqli_query($con, $sql);
						while($dskhoa = mysqli_fetch_array($khoa)){
							?>
						<option class="option" value="<?php echo $dskhoa['makhoa'];?>" <?php if(isset($_GET['khoa']) && $_GET['khoa']==$dskhoa['makhoa']) echo "selected"; ?>><?php echo $dskhoa['tenkhoa'];?></option>
					<?php	
						}
					 ?>
				...
				<select id="loai" name="loai">
					...
					<?php 
						...
						$loaitailieu = mysqli_query($con, $sql);
						while($dsltl = mysqli_fetch_array($loaitailieu)){
							?>
						<option class="option" value="<?php echo $dsltl['maloai']?>" <?php if(isset($_GET['loai']) && $_GET['loai']==$dsltl['maloai']) echo "selected"; ?>><?php echo $dsltl['tenloai']?></option>
						<?php
						}
					 ?>
				</select>

				<label for="domoi">Độ mới: </label>
				<select id="domoi" name="domoi">
					<option class="option" value="all">Tất cả</option>
					<option class="option" value="100" <?php if(isset($_GET['domoi']) && $_GET['domoi']==100) echo "selected"; ?>>100% - Chưa sử dụng</option>
					<option class="option" value="80" <?php if(isset($_GET['domoi']) && $_GET['domoi']==80) echo "selected"; ?>>Từ 80% - Rất tốt</option>
					<option class="option" value="65" <?php if(isset($_GET['domoi']) && $_GET['domoi']==65) echo "selected"; ?>>Từ 65% - Khá tốt</option>
					<option class="option" value="50" <?php if(isset($_GET['domoi']) && $_GET['domoi']==50) echo "selected"; ?>>Từ 50% - Tương đối</option>
					<option class="option" value="0" <?php if(isset($_GET['domoi']) && $_GET['domoi']==0) echo "selected"; ?>>Dưới 50% - Kém</option>
				</select>

				<label for="tacgia">Tác giả: </label>
				<input type="text" name="tacgia" id="tacgia" <?php if(isset($_GET['tacgia'])) echo "value=\"".$_GET['tacgia']."\""; ?>>

				<input type="submit" name="filter-submit" value="Lọc" id="filter-submit">
			</form>
		</div>
							...
							<div class="product">
								...
								<div class="pro-info">
									...
									<div class="pro-button">
										...
										<input type="button" class="button-2" id="b<?php echo $matailieu; ?>" name="submit" value="Trao đổi ngay" onclick="<?php if ($_SESSION['user_valid']==false) echo "updateInfo();"; else echo "fexchange($matailieu)"; ?>">
										<input type="submit" name="exchange" id="exchange<?php echo $matailieu; ?>" value="exchange" class="hidden">
									</div>	
								</div>
