<?php 
	...
	$sql = "SELECT * FROM `nguoidung` WHERE `username` LIKE '".$_POST['username']."';";
	...
	<div id="main">
		...
		<div id="left">	
			...
			<div id="seller">
				...
				<p class="subp"><span>Gmail: </span> <i>Đã được ẩn</i></p>
				<p class="subp"><span>Facebook: </span> <i>Đã được ẩn</i></p>
				<p class="subp"><span>Zalo: </span> <i>Đã được ẩn</i></p>
		...
		<div id="right">
			<ul id="tag">
				...
				<li id="li2" onclick="shared(); page('first','b');">Chia sẻ thành công</li>
			</ul>
			<div id="seller-pro">
				<div id="sharing">
					<?php 
						$sql = "SELECT * FROM `tailieu` WHERE `matrangthai` LIKE 'sharing' AND `username` LIKE '".$nguoidung['username']."';";
						...
						?>	
								...
								<form method="GET" action="../Form/exchange.php">
									<div class="product" id="product-a<?php echo $pid;?>" <?php if($pid>3) echo "style=\"display: none;\""; ?>>
										...
										<div class="pro-info">
											...
											<?php if($nguoidung['username']!=$_SESSION['username']){ ?>
					...
					<div class="page-num">
						<?php 
							if (mysqli_num_rows($tailieu) == 0) {
								echo "Người dùng này hiện đang không chia sẻ tài liệu nào cả!";
							}else{
				...
				<div id="shared">
					<?php 
						$sql = "SELECT * FROM `tailieu` WHERE `matrangthai` LIKE 'shared' AND `username` LIKE '".$nguoidung['username']."';";
						$tailieu = mysqli_query($con, $sql);
						$pid=0;
						while($dstl = mysqli_fetch_array($tailieu)){
							$pid++;
					...
					<div class="page-num">
						<?php 
							if (mysqli_num_rows($tailieu) == 0) {
								echo "Người dùng này hiện chưa chia sẻ thành công tài liệu nào!";
							}else{
								if(mysqli_num_rows($tailieu) > 3){
						 ?>
